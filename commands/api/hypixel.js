  ...
  async execute(message, args) {
    ...
    if (args[0] === "auctionhouse" || args[0] === "ah") {
      ...
      var search = items.filter(item => {
        const searched = item.name.toLowerCase().search(args.slice(1).join(" ").toLowerCase());
        return searched !== -1;
      });
      ...
      function compare(a, b) {
        if ((a.highest_bid_amount > 0 ? a.highest_bid_amount : a.starting_bid) / a.item.count < (b.highest_bid_amount > 0 ? b.highest_bid_amount : b.starting_bid) / b.item.count) return -1;
        if ((a.highest_bid_amount > 0 ? a.highest_bid_amount : a.starting_bid) / a.item.count > (b.highest_bid_amount > 0 ? b.highest_bid_amount : b.starting_bid) / b.item.count) return 1;
        return 0;
      }
      auctions.sort(compare);
      const lowest = Math.max(auctions[0].highest_bid_amount, auctions[0].starting_bid) / auctions[0].item.count;
      const highest = Math.max(auctions[auctions.length - 1].highest_bid_amount, auctions[auctions.length - 1].starting_bid) / auctions[auctions.length - 1].item.count;
      var average = 0;
      for (const i of auctions) average += Math.max(i.highest_bid_amount, i.starting_bid) / i.item.count;
      ...
      if (!args[1]) return message.channel.send("Please enter an item!");
      const productIdsAPI = await fetch(`https://api.hypixel.net/skyblock/bazaar/products?key=${process.env.API}`).then(resp => resp.json());
      const productIds = productIdsAPI.productIds;
      for (const id of ["OAK_LOG", "BIRCH_LOG", "SPRUCE_LOG", "DARK_OAK_LOG", "ACACIA_LOG", "JUNGLE_LOG", "COCOA_BEANS", "LAPIS_LAZULI", "RAW_SALMON", "PUFFERFISH"]) productIds.push(id);
      var firstPrior = productIds.filter(item => item.toLowerCase().replace(/_/g, " ").search(args.slice(1).join(" ").toLowerCase()) === 0);
      var search = productIds.filter(item => {
        var searched = item.toLowerCase().replace(/_/g, " ").search(args.slice(1).join(" ").toLowerCase());
        return searched !== -1;
      });
      if (search.size === 0 && firstPrior.size == 0) return message.channel.send(`No item found for \`${args.slice(1).join(" ")}\``);
      ...
      const res = await nameToUuid(args[0]);
      ...
      try {
        ...
        else if (body.player.prefix === "§d[PIG§b+++§d]") rank = "[PIG+++]";
        else if (body.player.rank === "YOUTUBER") rank = "[Youtuber]";
        else if (body.player.newPackageRank === "VIP") rank = "[VIP]";
        else if (body.player.newPackageRank === "VIP_PLUS") rank = "[VIP+]";
        else if (body.player.newPackageRank === "MVP") rank = "[MVP]";
        else if (body.player.newPackageRank === "MVP_PLUS") rank = "[MVP+]";
        ...
        const Embed = new Discord.MessageEmbed()
          ...
          .setThumbnail("https://cdn.glitch.com/0ee8e202-4c9f-43f0-b5eb-2c1dacae0079%2Fhypixel.jpg?v=1579257362271")
          .addField("Rank", rank2, true)
          .addField("Level", level, true)
          .addField("Minecraft Version", body.player.mcVersionRp, true)
          .addField("Guild", "No guild", true)
          .addField("Karma", numberWithCommas(karma), true)
          .addField("First/Last login", "`" + firstlogin + " | " + lastlogin + "`", true)
          .setImage(skin)
          .setTimestamp()
          .setFooter("Have a nice day! :)", message.client.user.displayAvatarURL());
        if (rank !== "Non") Embed.setTitle(rank + res[0].name)
        if (stuff.guild) {
    ...
    if (args[0] === "guild" || args[0] === "g") {
      ...
      const guildCoins = guBody.guild.coins.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
      const guildCreated = new Date(guBody.guild.created);
      const readableTime = readableDateTime(guildCreated);
      const member = guBody.guild.members;
      function compare(a, b) {
      ...
      const members = member.sort(compare).map(x => {
        return {
          uuid: x.uuid,
          name: null,
      ...
      for (var i = 0; i < 10; i++) {
        const result = await fetch(`https://api.minetools.eu/uuid/${members[i].uuid}`).then(res => res.json());
      ...
      const Embed = new Discord.MessageEmbed()
        ...
        .addField("Guild member count", member.length, true)
        .addField("Created date", `\`${readableTime}\``, true)
        .setTimestamp()
        .setFooter("Have a nice day! :)", message.client.user.displayAvatarURL());
      ...
      var wins_capture = 0;
      var kills_capture = 0;
      if (tnt.coins) coins = tnt.coins;
      if (tnt.deaths_bowspleef) deaths_bowspleef = tnt.deaths_bowspleef;
      if (tnt.wins_bowspleef) wins_bowspleef = tnt.wins_bowspleef;
      if (tnt.wins_tntag) wins_tntag = tnt.wins_tntag;
      if (tnt.wins_tntrun) wins_tntrun = tnt.wins_tntrun;
      if (tnt.record_tntrun) record_tntrun = Math.floor(tnt.record_tntrun / 60) + ":" + ("0" + (tnt.record_tntrun % 60)).slice(-2);
      if (tnt.deaths_capture) deaths_capture = tnt.deaths_capture;
      ...
      const Embed = new Discord.MessageEmbed()
        ...
        .setThumbnail("https://cdn.glitch.com/0ee8e202-4c9f-43f0-b5eb-2c1dacae0079%2FTNT.png?v=1579257361129")
